function [Basis,D_Basis,DD_Basis,ClampedBasis,D_ClampedBasis,DD_ClampedBasis,ClampedBMBBasis,D_ClampedBMBBasis,DD_ClampedBMBBasis,xarr,x_spacing] = cubicbspline(a,b,num_splines)


h=(b-a)/(num_splines-1);
xi=0;
x1=linspace(-2*h,xi-h,5001);
x2=linspace(xi-h,xi,5001);
x3=linspace(xi,xi+h,5001);
x4=linspace(xi+h,xi+2*h,5000);
x=[x1 x2 x3 x4];
...
Basis=zeros(13,length(y)/4*(num_splines-1)); %Bbasis is Bspline with the two extra splines deleted
...
ClampedBMBBasis((num_splines-1)/2,:)=ClampedBasis((num_splines-1)/2,:)+ClampedBasis((num_splines-1)/2-1,:)+ClampedBasis((num_splines-1)/2+1,:);
...
D_ClampedBMBBasis((num_splines-1)/2,:)=D_ClampedBasis((num_splines-1)/2,:)+D_ClampedBasis((num_splines-1)/2-1,:)+D_ClampedBasis((num_splines-1)/2+1,:);
for i=1:((num_splines-1)/2-1)
...
%BMB Basis Vectors with Free ends. Zero slope condition at mass location.
BMBBasis=zeros(num_splines,length(xarr));
BMBBasis((num_splines-1)/2,:)=Basis((num_splines-1)/2,:)+Basis((num_splines-1)/2-1,:)+Basis((num_splines-1)/2+1,:);
for i=1:((num_splines-1)/2-1)
    BMBBasis(i,:)=Basis(i,:);
end
for i=((num_splines-1)/2+1):num_splines
