function [Basis,D_Basis,DD_Basis,ClampedBasis,D_ClampedBasis,DD_ClampedBasis,ClampedBMBBasis,D_ClampedBMBBasis,DD_ClampedBMBBasis,BMBBasis,D_BMBBasis,DD_BMBBasis,xarr,x_spacing] = cubicbspline2(a,b,num_splines)
...
BMBBasis=1/4*Basis;
BMBBasis((num_splines+3)/2,:)=1/6*(Basis((num_splines+3)/2+1,:)+Basis((num_splines+3)/2-1,:)+Basis((num_splines+3)/2,:));
BMBBasis((num_splines+3)/2-1,:)=[];
BMBBasis((num_splines+3)/2,:)=[];

D_BMBBasis=1/4*D_Basis;
D_BMBBasis((num_splines+3)/2,:)=1/6*(D_Basis((num_splines+3)/2+1,:)+D_Basis((num_splines+3)/2-1,:)+D_Basis((num_splines+3)/2,:));
D_BMBBasis((num_splines+3)/2-1,:)=[];
D_BMBBasis((num_splines+3)/2,:)=[];

DD_BMBBasis=1/4*DD_Basis;
DD_BMBBasis((num_splines+3)/2,:)=1/6*(DD_Basis((num_splines+3)/2+1,:)+DD_Basis((num_splines+3)/2-1,:)+DD_Basis((num_splines+3)/2,:));
DD_BMBBasis((num_splines+3)/2-1,:)=[];
DD_BMBBasis((num_splines+3)/2,:)=[];

%Uncomment to clamp mass location 

%BMB Basis Vectors with Free ends. Zero slope and position condition at mass location.
% BMBBasis=Basis;
%  BMBBasis((num_splines+3)/2,:)=-2*Basis((num_splines+3)/2+1,:)-2*Basis((num_splines+3)/2-1,:)+Basis((num_splines+3)/2,:);
%  BMBBasis((num_splines+3)/2-1,:)=[];
%  BMBBasis((num_splines+3)/2,:)=[];
% 
% D_BMBBasis=D_Basis;
%  D_BMBBasis((num_splines+3)/2,:)=-2*D_Basis((num_splines+3)/2+1,:)-2*D_Basis((num_splines+3)/2-1,:)+D_Basis((num_splines+3)/2,:);
%  D_BMBBasis((num_splines+3)/2-1,:)=[];
%  D_BMBBasis((num_splines+3)/2,:)=[];
% 
% DD_BMBBasis=DD_Basis;
%  DD_BMBBasis((num_splines+3)/2,:)=-2*DD_Basis((num_splines+3)/2+1,:)-2*DD_Basis((num_splines+3)/2-1,:)+DD_Basis((num_splines+3)/2,:);
%  DD_BMBBasis((num_splines+3)/2-1,:)=[];
%  DD_BMBBasis((num_splines+3)/2,:)=[];
