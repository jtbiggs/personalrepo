...
mass_size=0.25;
w0=@(x) -0.5*x; %initial position at t=0
wdot0 = @(x) 0; %initial velocity at t=0
num_b_splines=15; 
...
for i=1:N
    ...
    W0(i,:)=trapz(xarr,w0(xarr).*ClampedBMBBasis(i,:));
    Wdot0(i,:)=trapz(xarr,wdot0(xarr).*ClampedBMBBasis(i,:));
end
...
alpha0=M\W0;
beta0=zeros(N,1); 
beta0=M\Wdot0; %initial vector (g = 0)
...
[t,alphac]=ode15s(@dbeam_lqr,tspan,y_init,[],A,K_lqr,B); %solves system of ordinary differential equations
